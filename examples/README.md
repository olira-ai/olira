...
- `13_outbound_actions.py` needs `OLIRA_ACTIONS_WEBHOOK_URL` set to a **public HTTPS** URL (localhost and private addresses are rejected); a https://webhook.site URL works for a quick look, or use your own ngrok tunnel or deployed receiver to verify the HMAC signature for real. It creates (and cleans up) a throwaway destination, and a throwaway patient unless `OLIRA_PATIENT_ID` is set.
