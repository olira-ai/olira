...
### Added
- Batch export APIs on `OliraClient`/`AsyncOliraClient`: `create_export()`, `get_export()`,
  `list_exports()`, `download_export()` — start a ZIP/Parquet export job, poll status, list
  history, and fetch a presigned download URL. Requires `sdk:state-read`. New models:
  `ExportInclude`, `ExportJob`, `ExportJobStatus`, `ExportJobListResult`, `ExportDownload`.

## [1.12.0] - 2026-08-10

### Added
- `list_log_types()` / `get_log_type()` on `OliraClient`/`AsyncOliraClient` — live discovery of the
  platform's log-type catalog, including each type's full payload JSON Schema. Requires
  `sdk:event-log` scope. Complements the static `OliraLogType` enum for agent-guided mapping.
